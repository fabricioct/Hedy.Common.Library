...
namespace Hedy.Common.Library.Extension
{
	public static class StringExtension
	{
		...
		{
			return value.IsNullOrWhiteSpace() ? null : Regex.Replace(value, @"[^\d]", string.Empty);
		}

		public static string TrimUpper(this string value)
		{
			return value.IsNullOrWhiteSpace() ? null : value.Trim().ToUpper();
		}
		...
		public static bool ValidateCpfOrCnpj(this string cpfCnpj)
		{
			var cpfCnpjValido = false;

			if (cpfCnpj.ValidateCnpj() || cpfCnpj.ValidateCpf())
				cpfCnpjValido = true;
